...
contract VesperPool_Owner_Test is VesperPoolTestBase {
    ...
    function test_migrateStrategy() public {
        ...
        assertEq(pool.getStrategies().length, 1);
        assertEq(pool.getWithdrawQueue().length, 1);
        assertEq(pool.getStrategies()[0], strategy);

        address _newStrategy = makeAddr("newStrategy");
        vm.mockCall(strategy, abi.encodeWithSelector(IStrategy.pool.selector), abi.encode(address(pool)));
        vm.mockCall(_newStrategy, abi.encodeWithSelector(IStrategy.pool.selector), abi.encode(address(pool)));

        pool.migrateStrategy(strategy, _newStrategy);
        assertFalse(pool.getStrategyConfig(strategy).active);
        VesperPool.StrategyConfig memory _config = pool.getStrategyConfig(_newStrategy);
        assertTrue(_config.active);
        assertEq(_config.debtRatio, debtRatio);
        assertEq(pool.getStrategies().length, 1);
        assertEq(pool.getWithdrawQueue().length, 1);
        assertEq(pool.getStrategies()[0], _newStrategy);
    }

    function test_migrateStrategy_revertWhen_callerIsNotOwner() public {
        assertNotEq(pool.owner(), alice);

        vm.expectRevert(abi.encodeWithSelector(Ownable.OwnableUnauthorizedAccount.selector, alice));
        vm.prank(alice);
        pool.migrateStrategy(strategy, strategy2);
    }

    function test_migrateStrategy_revertWhen_newStrategyIsAlreadyActive() public {
        pool.addStrategy(strategy, debtRatio);
        address _newStrategy = makeAddr("newStrategy");
        pool.addStrategy(address(_newStrategy), debtRatio2);
    ...
    function test_migrateStrategy_revertWhen_oldStrategyIsNotActive() public {
        address _newStrategy = makeAddr("newStrategy");
    ...
    function test_migrateStrategy_revertWhen_strategyIsInvalid() public {
        pool.addStrategy(strategy, debtRatio);
        assertTrue(pool.getStrategyConfig(strategy).active);
        vm.mockCall(strategy, abi.encodeWithSelector(IStrategy.pool.selector), abi.encode(address(pool)));

        // notice this strategy has 0x1 as pool
        address _newStrategy = makeAddr("newStrategy");
        vm.mockCall(_newStrategy, abi.encodeWithSelector(IStrategy.pool.selector), abi.encode(address(0x1)));

        vm.expectRevert(VesperPool.InvalidStrategy.selector);
        pool.migrateStrategy(strategy, _newStrategy);
    }
